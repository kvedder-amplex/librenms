...
foreach ($pre_cache['ciena_oids'] as $index => $entry) {
	...
	$oid = '.1.3.6.1.4.1.6141.2.60.4.1.1.1.1.105.'.$index;
        ...
        discover_sensor($valid['sensor'], 'dbm', $device, $oid, 'rx-'.$index, 'ciena-sds', $descr, $divisor, $multiplier, $limit_low, null, null, $limit, $current, 'snmp', $entPhysicalIndex, $entPhysicalIndex_measured);
    }

    if (is_numeric($entry['wwpLeosPortXcvrTxDbmPower']) && $entry['wwpLeosPortXcvrTxDbmPower'] && $entry['wwpLeosPortXcvrRxDbmPower']) {
        ...
        $entPhysicalIndex = $index;
        $entPhysicalIndex_measured = 'ports';
        discover_sensor($valid['sensor'], 'dbm', $device, $oid, 'tx-'.$index, 'ciena-sds', $descr, $divisor, $multiplier, $limit_low, null, null, $limit, $current, 'snmp', $entPhysicalIndex, $entPhysicalIndex_measured);
    }
}
